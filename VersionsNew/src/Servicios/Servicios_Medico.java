...
public class Servicios_Medico {
    ...
    public Medico getByName(Connection conexion, String nombre) throws SQLException{
        Medico medico = null;
        try{
            PreparedStatement consulta = conexion.prepareStatement("SELECT codigo, nombre, apellidos FROM "+this.tabla+" WHERE nombre=?");
            consulta.setString(1, nombre);
            ResultSet resultado = consulta.executeQuery();
            while(resultado.next()){
                medico = new Medico(resultado.getInt("codigo"), resultado.getString("nombre"), resultado.getString("apellidos"));
            }
        }catch(SQLException ex){
            throw new SQLException(ex);
        }
